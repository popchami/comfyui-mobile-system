...
These TODOs are **post-RunPod-and-Android-validation work**.

Do not start these items until the PR passes the remaining real-environment checks described in:
...
```text
Before RunPod + Android validation: fix only blockers, docs, and shape mismatches.
After RunPod + Android validation: revisit and reprioritize this file.
```

## Do not mix with current validation

The following items are important, but they should not block the next RunPod + Android validation pass:
...
```text
Changing storage before real Android validation can introduce Android path/permission issues. First prove MVP save/load works.
```
...
- Runtime test with real RunPod ComfyUI.
- Android real-device or emulator test.
...
```text
MVP already attempts re-run behavior. Polish and edge cases should wait until the first real RunPod + Android proof succeeds.
```
...
## Post-validation suggested priority

After RunPod + Android validation passes, use this suggested order:
...
7. Add bypass handling for optional branches.
8. Add subgraph handling.
9. Add node color matching.
```

Reason:

```text
Re-run, image handling, and error clarity are core app behavior. Runtime checks improve reliability. UI conversion expands compatible workflows. Bypass and subgraph support affect correctness. Node color improves usability but can come after functional safety.
```
