...
## セッション開始時の必須動作
新しいセッションが開始したら、他の作業より先に必ず
docs/HANDOFF.md を読み込み、「進行中・次にやること」
「ブロック中・保留」の内容を確認してから作業を開始すること。
作業が一段落したら、docs/HANDOFF.md を最新の状態に
全文書き換えて更新すること(差分追記ではなく、
完了済み/進行中/保留の各セクションを丸ごと書き直す)。
